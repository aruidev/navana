...
if (!isset($_SESSION['user_id'])) {
    sendJsonError('unauthorized', 'Authentication required.', 401);
}
...
if ($route === 'api/v1/user/bookmarks/item') {
    ...
    if ($method === 'POST') {
        $savedItemService->saveItem($userId, $itemId);
        sendJson([
            'data' => [
                'itemId' => $itemId,
                'saved' => true,
    ...
    sendJsonError('method_not_allowed', 'Only POST and DELETE are allowed for this endpoint.', 405);
}
...
$result = $savedItemService->getSavedItemsPaginated($userId, $page, $perPage, $term, $order);
$items = array_map(
